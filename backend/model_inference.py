...
from face_box.boundingbox import load_model as face_box_load
...
class ProjectionHead(nn.Module):
    def __init__(self, input_dim, proj_dim):
        ...
        self.projector = nn.Sequential(
            nn.Linear(input_dim, 512), nn.ReLU(), nn.Linear(512, proj_dim)
        )
    ...
    def forward(self, cropped_faces, audio_features):
        ...
        if isinstance(cropped_faces, torch.Tensor):
            if cropped_faces.dim() == 4:  # [B, 3, 224, 224]
                ...
                mobilenet_batch = F.interpolate(
                    cropped_faces, size=(224, 224), mode="bilinear", align_corners=False
                )

                mean = torch.tensor(
                    [0.485, 0.456, 0.406], device=cropped_faces.device
                ).view(1, 3, 1, 1)
                std = torch.tensor(
                    [0.229, 0.224, 0.225], device=cropped_faces.device
                ).view(1, 3, 1, 1)
                ...
                face_points_batch = torch.tensor(
                    face_points_list, device=device, dtype=torch.float32
                )

            else:
                raise ValueError(
                    f"Expected 4D tensor for batch processing, got {cropped_faces.dim()}D"
                )
        ...
        visual_feature = torch.cat(
            [img_features, face_points_batch], dim=1
        )  # [B, 1024 + 136]
        visual_proj_input = torch.cat(
            [visual_feature, eye_features], dim=1
        )  # [B, 1024+136+2048=~4208]
